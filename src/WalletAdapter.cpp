...
void WalletAdapter::updateBlockStatusText() {
  ...
  if (!encrypted) {
    walletSecurity = "Wallet Status: Unencrypted";
  } else
  {
  ...
  const QString statusString = blockTime.msecsTo(currentTime) < LAST_BLOCK_INFO_WARNING_INTERVAL ? "Sychronized with the Network" : "WARNING";
